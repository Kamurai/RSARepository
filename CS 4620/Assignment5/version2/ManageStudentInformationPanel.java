...
public class ManageStudentInformationPanel extends JPanel
{
	...
	public ManageStudentInformationPanel()
	{
	...
	private class UpdateStudentListener implements ActionListener
	{
		public void actionPerformed (ActionEvent event)
		{
			...
			int index = DataController.getIndex(Output2.getText());
			...
			DataController.updateCareerInterests(index, CI);
		...
		public void actionPerformed (ActionEvent event)
  		{
			...
			for(int x = 0; x < DataController.getSize(); x++)
			{
				...
				if( event.getSource() == buttonlist.get(x) )
				{
					...
					Output2.setText(DataController.getStudent(x).getStudentName());
